...
4. **Answer Phase 0.6(b):** can `POST /1.1/account/update_profile` be called on this plan? Item 7 (the bio noticeboard) is built only if it can, and it is not started. It is a Developer Console question and writing to the live profile is not something that can be tested without changing it.
