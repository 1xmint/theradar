...
| USDC-quoted pump.fun coins | **measured 2026-09-09, and wider than USDC** | ~~unverified~~. Settled by capture: of ten PumpSwap pools read from mainnet, one quotes in USDC and five quote in other SPL mints including another `…pump` token. See [research 0033](../research/0033-the-pumpswap-pool-account-has-eight-lengths.md) and [`pumpswap_pools.json`](../../crates/radar-pumpfun/tests/fixtures/pumpswap_pools.json). The two vendor pages that disagreed are both wrong to read as SOL-only |
...
| USDC-quoted curves | **the capture exists** as of 2026-09-09, on the pool side: [`pumpswap_pools.json`](../../crates/radar-pumpfun/tests/fixtures/pumpswap_pools.json) holds a USDC-quoted PumpSwap pool and five quoted in other SPL mints. The refusal below is therefore no longer a placeholder for a missing capture — it stays because **a non-SOL quote still has no dollar value without a chain of prices**, and a pool quoted in another memecoin needs more than one link. The curve side is still uncaptured | done for pools; the curve side, or a documented refusal with the reason on the sheet |
