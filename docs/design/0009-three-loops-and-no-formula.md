...
| The admission gate | built and pure: a per-summoner daily cap, a global daily cap, a dedupe window, an ignore list, and **refuses everything when unconfigured** — [`admission.rs`](https://github.com/1xmint/realorrug/blob/main/crates/realorrug-analyst/src/admission.rs) (moved to [1xmint/realorrug](https://github.com/1xmint/realorrug) on 2026-09-14) |
...
[`realorrug:crates/realorrug-analyst/src/mention.rs`](https://github.com/1xmint/realorrug/blob/main/crates/realorrug-analyst/src/mention.rs)),
...
| M5 | **Telegram publisher.** Long-poll `getUpdates`; each message → `mention::read` → `Gate::admit` → dossier → roast → log → reply. Unset token ⇒ nothing (rule 8). Not a contest entry, not in the record. **Built 2026-09-05** as [`realorrug:crates/realorrug-analyst/src/telegram.rs`](https://github.com/1xmint/realorrug/blob/main/crates/realorrug-analyst/src/telegram.rs) (moved to [1xmint/realorrug](https://github.com/1xmint/realorrug) on 2026-09-14): its own log file is what keeps it out of the record; a second switch, `RADAR_TELEGRAM_PUBLISH=on`, keeps it silent until read | parser, gate, fact path, `Publisher` trait in [`realorrug:crates/realorrug-analyst/src/publish.rs`](https://github.com/1xmint/realorrug/blob/main/crates/realorrug-analyst/src/publish.rs) | nothing further; the env section is now in realorrug's own `deploy/analyst.env.example` and `radar brief` counts the lane | `deploy/README.md` | a bot token from Josh via BotFather, five minutes. A **different** bot from the alert one |
...
| M4 | **"Seven days later."** Once a day: replies seven days old from the log, joined once against the store's outcomes on the creator-index timer's pattern, rendered as one post to X and Telegram. **Built 2026-09-05**: the join is `radar seven-days-later` on [`deploy/radar-seven-days.timer`](../../deploy/radar-seven-days.timer) (still built here; only the posting daemon moved), the post was [`realorrug:crates/realorrug-analyst/src/daily.rs`](https://github.com/1xmint/realorrug/blob/main/crates/realorrug-analyst/src/daily.rs) at 12:00 UTC, under the same two checks | the reply log (read as a file since 2026-09-14 by [`radar_backfill::analyst_log`](../../crates/radar-backfill/src/analyst_log.rs), which replaced [`realorrug:crates/realorrug-analyst/src/log.rs`](https://github.com/1xmint/realorrug/blob/main/crates/realorrug-analyst/src/log.rs)); the outcomes; the timer pattern in [`deploy/radar-creator-index.service`](../../deploy/radar-creator-index.service) | nothing further | — | the bot live for seven days. Day one has nothing to say and says so |
| M2 | **The weekly teardown post.** The winner's mint gets the full dossier as the week's top-level post; the vault, the prize and the transaction in the same post (C8). **Built 2026-09-05** as [`realorrug:crates/realorrug-analyst/src/weekly.rs`](https://github.com/1xmint/realorrug/blob/main/crates/realorrug-analyst/src/weekly.rs) over the record [`realorrug:crates/realorrug-analyst/src/contest.rs`](https://github.com/1xmint/realorrug/blob/main/crates/realorrug-analyst/src/contest.rs) closes: the result under 280 characters as the top-level post and the teardown as its reply, the winner named by the reply's URL rather than a handle (the log carries ids, not names) | the week-close job and the roaster | nothing further | — | M1 |
| M3 | **The hunter rank.** Status only. Rule below | the signals are on the sheet ([`realorrug:crates/realorrug-roast/src/sheet.rs`](https://github.com/1xmint/realorrug/blob/main/crates/realorrug-roast/src/sheet.rs)) — **counted there as a type since 2026-09-05** (`Signal`, three variants, the strongest band read off the snapshot rather than named) and carried on every reply log entry; the tally is [`realorrug:crates/realorrug-contest/src/hunter.rs`](https://github.com/1xmint/realorrug/blob/main/crates/realorrug-contest/src/hunter.rs); the per-summoner cap is in the gate and, since the same day, charged on admission so a burst cannot read the chain past it | the week-close job that feeds the log to the tally (plan 0006 item 6); a second tab on [`site/src/Leaderboard.tsx`](../../site/src/Leaderboard.tsx) | — | M1. The rule below is accepted by the same delegation as L1–L6, with its change policy |
