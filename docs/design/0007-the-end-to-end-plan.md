...
| F5 | ~~**Branch hygiene**: 116 remote branches~~ **Withdrawn 2026-09-04: the premise was wrong.** That count came from `git branch -r` against a checkout that had not pruned, so it was counting **stale local refs to branches already deleted on the remote**. After `git fetch --prune` there are **18**, of which 16 have merged pull requests and the other two are `main` and a branch whose commits are already in `main`. Nothing is stranded and nothing needs deleting — `delete_branch_on_merge` has been doing its job. Kept rather than removed, because "we counted the wrong thing" is the more useful record. | `git fetch --prune && git branch -r` |
