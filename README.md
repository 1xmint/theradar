...
| `radar-payout` | Pays the week's prize: `collect_creator_fee` and one transfer to the claimed address, under the contest's three refusals, then reads the transaction back before the ledger says paid. Its own key, unit and user; not the trading signer, and cannot sign a trade. The manual fallback goes through the same read-back. |
...
| `radar-analyst` | The summoned-reply loop, and the daemon that runs it. Only a base58 mint or a `$TICKER` survives parsing a mention, so there is no field an instruction can travel in; an admission gate caps chain reads per summoner and replies per day and per mint; a spend meter reserves before every call and refuses everything when unfunded; every reply is recorded **before** it is said and again after, so a published statement always has a record. The X client exists and posts nothing without `RADAR_X_BEARER` and `RADAR_X_USER_ID` — with neither set the publisher is a dry run, which is what ships. A Telegram lane runs beside it on its own token, caps and log file — the free lane of design 0009 L5, kept out of the contest by never being in the file the contest reads. The week closes here too — replies scored on **verified** engagement where the scan reached an entry and on raw metrics where it did not, the record written where the site reads it — and the account's posts go out under the same fidelity and forbidden checks a reply passes: the week's result as a thread (the summary, the winner's coin torn down, the week's best hunters), and the daily "seven days later". A model provider writes the reply when one is configured and the deterministic template ships when one is not, with every number checked back against the fact sheet either way. |
| `radar-contest` | The weekly contest, pure. Weeks that open Monday 00:00 UTC, the published score over the bot's **own** replies, every exclusion returned with its reason rather than dropped, a winner with a cooldown, the seven-day claim, one JSON record per week, and the payout policy `radar-payout` must ask before it signs — six refusals now, ordered so the most urgent answer is the one returned: already paid, voided, no winner, unclaimed, wrong recipient, above what was collected, and last of all below the floor. The rule each week was scored under is written into that week's record, because a closed week has to stay checkable against the rule that actually decided it. The hunter rank sits beside the prize: status, never money. No clock, no network, no key. |
| `radar-journal` | What the autonomous surface decided, written **before** it acted. One JSON object per line, hash-chained: each event carries the id of the one before it, so an alteration is visible from a later event — relative to a trusted checkpoint, which is what a chain buys and all it buys. Reading one back keeps three answers apart that an operator must not confuse: nothing was ever written, the last write was interrupted, and something wrote to this file that was not the journal. A write that fails blocks the effect it was recording. ADR 0017. |
